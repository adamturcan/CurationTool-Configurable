...
/**
 * Translation orchestration. Covers whole-document translate, per-segment translate, update, and delete.
 *
 * Each method takes a session snapshot, calls the translate endpoint, recomputes spans (shift or remove on length change), and returns a `translationsPatch` in a `WorkflowResult`.
 *
 ...
 *
 * Note on segmented translation forward-shift: it retranslates the merged source via API and overwrites the dictionary entry, so manually edited translation prose is silently lost. The action-guard exists to warn the user before this happens.
 *
