...
![Combined reproduction stack: `docker compose -f docker-compose.postgres.yml ps` lists all five services (postgres, memorise-api, memorise-ui, legal-mock, medical-mock) running under a single Compose file. Postgres reports `(healthy)`. Evidence that the entire configurability validation is reproducible from one declarative artefact.](images/01-postgres-compose-ps.png){ width=100% }
