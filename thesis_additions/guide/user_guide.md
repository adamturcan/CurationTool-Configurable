...
![Login screen in standalone mode](images/01-login-standalone.png){ width=85% }
...
![Login screen in platform mode](images/02-login-platform.png){ width=70% }

To create a new account, switch to the registration tab and supply a username, an optional email, and a password.

![Registration tab](images/03-register.png){ width=70% }
...
![Workspace list with pre-seeded workspaces](images/04-workspace-list.png){ width=100% }
...
![Create workspace dialog with the optional file-upload field](images/05-create-workspace.png){ width=70% }
...
![Inline rename with duplicate warning](images/06-rename-duplicate.png){ width=100% }
...
![Export workspace dialog](images/07-export-dialog.png){ width=60% }
...
![Delete confirmation](images/08-delete-confirm.png){ width=60% }
...
![The editor view](images/09-editor-overview.png){ width=100% }
...
![Empty editor region for source text entry](images/10-paste-text.png){ width=100% }
...
![Merge icon (left) and drag handle (right) between two segments](images/11-segment-icons.png){ width=35% }
...
![Floating split menu after selecting a delimiter character](images/27-split-menu.png){ width=60% }
...
![Action guard dialog when shifting across an untranslated segment](images/12-action-guard.png){ width=85% }
...
![Segment with highlighted entities and per-segment header](images/13-segment-toolbar.png){ width=100% }

The two coloured buttons on the right are, in order, the per-segment NER trigger and the per-segment Tag trigger.

![Per-segment NER and Tag buttons](images/14-ner-tag-buttons.png){ width=20% }
...
![Entity-type picker after selecting a fresh range of text](images/28-entity-picker.png){ width=50% }
...
![Edit menu after clicking an existing entity highlight](images/29-entity-edit-menu.png){ width=50% }
...
![Conflict resolution dialog](images/19-conflict.png){ width=90% }
...
![Language tab and add-language control](images/15-language-tab.png){ width=30% }

The picker is a searchable list of supported target languages, populated from the configured translate service.

![Adding a new target language](images/16-language-search.png){ width=40% }
...
![Switching between Original and existing translations](images/17-language-switcher.png){ width=35% }
...
![Edited indicator on a translated segment](images/18-edited-indicator.png){ width=30% }
...
![Re-translation warning for an edited segment](images/20-retranslate-warning.png){ width=85% }
...
![Tag panel with thesaurus search active](images/21-tag-search.png){ width=45% }
...
![Segment tags panel populated with selected entries](images/22-tag-panel.png){ width=50% }

### 8.3 Adding a free-text tag

Toggling the **Free Text** button at the top of the panel changes the input from thesaurus search to a literal-tag input, so the curator can attach an ad-hoc label that is not present in the thesaurus. The button alternates between **Thesaurus** and **Free Text** modes on each click.

![Free-text tag mode toggle](images/23-free-text-tag.png){ width=50% }
...
![Export controls on the global toolbar](images/24-export-icons.png){ width=20% }
...
![Admin Services panel with endpoint table](images/25-admin-panel.png){ width=100% }
...
![Endpoint table in edit mode with the adapter dropdown open](images/26-admin-edit.png){ width=100% }
...
If the curator attempts to navigate away or close a workspace with unsaved changes still in flight, the application interposes an unsaved-changes dialog with **Save & Leave**, **Leave without saving**, and **Cancel** options, so that no work is lost to an accidental click.
